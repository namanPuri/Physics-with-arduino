...
To plot curve for temperature plotting
'''
...
def main(queue: Queue) -> None:
    ...
    line, = ax.plot([], marker='o', lw=0, markersize=2)

    ax.set_xlim([0, 3])
    ax.set_ylim([-10,200])

    fig.suptitle("Newtons law of cooling", fontsize=15)
    ...
    while True:
        ...
        ax.set_xlim(min(x_adjusted) - max(x_adjusted)*0.01,max(x_adjusted)*1.01)
